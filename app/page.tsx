...
interface Profile {
  role: 'superadmin' | 'advisor' | 'client';
  site_id: string | null;
  organisation_id: string | null;
}
  ...
  return (
    <div className={`rounded-2xl border transition-all duration-300 overflow-hidden ${isResolved ? 'bg-slate-50/60 border-slate-100 opacity-60' : `${cfg.bg} ${cfg.border}`}`}>
      <div className="p-6 flex flex-col md:flex-row md:items-start gap-4">
        <div className={`w-1.5 rounded-full self-stretch hidden md:block flex-shrink-0 transition-colors ${isResolved ? 'bg-slate-300' : cfg.bar}`} style={{ minHeight: 64 }} />
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            ...
            <span className={`text-[10px] font-black uppercase px-3 py-1.5 rounded-lg border flex-shrink-0 ${isResolved ? 'bg-white border-slate-200 text-slate-400' : `border ${cfg.badge}`}`}>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <button onClick={() => setExpanded(e => !e)} className="p-2.5 rounded-xl bg-white/80 border border-white/60 text-slate-400 hover:text-indigo-600 hover:border-indigo-200 transition-all shadow-sm">
            {expanded ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
          </button>
          <button onClick={() => onToggleResolve(action.id)}
            className={`px-5 py-2.5 rounded-xl font-black text-xs uppercase tracking-wider transition-all active:scale-95 shadow-sm flex items-center gap-2 ${isResolved ? 'bg-white border border-slate-200 text-slate-400' : 'bg-slate-900 text-white hover:bg-indigo-700'}`}>
        ...
        <div className="border-t border-white/60 bg-white/60 backdrop-blur-sm px-6 py-5 space-y-5">
            ...
            <div>
              ...
              {action.evidenceLabel ? (
                <div className="bg-white rounded-xl border border-indigo-100 px-4 py-3 flex items-center gap-3 cursor-pointer hover:border-indigo-300 transition-colors group">
                  <FileText size={16} className="text-indigo-400 group-hover:text-indigo-600 flex-shrink-0" />
                  <span className="text-xs font-bold text-indigo-600 truncate">{action.evidenceLabel}</span>
                  <ExternalLink size={12} className="text-slate-300 group-hover:text-indigo-400 ml-auto flex-shrink-0" />
                </div>
              ) : (
                <div className="bg-white rounded-xl border border-dashed border-slate-200 px-4 py-3 flex items-center justify-center gap-2 cursor-pointer hover:border-indigo-300 transition-colors group">
                  ...
                  <span className="text-xs font-bold text-slate-300 group-hover:text-indigo-400">Upload Evidence</span>
                </div>
              )}
            </div>
          </div>
          {showNoteInput ? (
            <div className="flex gap-2 items-start">
              <textarea value={noteText} onChange={e => setNoteText(e.target.value)} placeholder="Add a progress note…" rows={2}
              ...
              <div className="flex flex-col gap-2">
                <button onClick={() => { onAddNote(action.id, noteText); setNoteText(''); setShowNoteInput(false); }}
                  className="px-4 py-2.5 bg-indigo-600 text-white rounded-xl text-xs font-black hover:bg-indigo-700">Save</button>
                <button onClick={() => setShowNoteInput(false)}
                  className="px-4 py-2.5 bg-white border border-slate-200 text-slate-400 rounded-xl text-xs font-black hover:bg-slate-50">Cancel</button>
              </div>
            </div>
          ) : (
            <button onClick={() => setShowNoteInput(true)} className="text-[11px] font-black uppercase tracking-wider text-indigo-500 hover:text-indigo-700 flex items-center gap-1.5">
...
function normaliseItems(raw: any): DattoItem[] {
  ...
  return list.map((item: any) => ({
    ...
    type: (item.type === 'folder' || item.type === 'FOLDER' || item.isDirectory === true ||
      item.folderType !== undefined || item.childCount !== undefined ||
      (item.mimeType && item.mimeType === 'application/vnd.datto.folder')
    ) ? 'folder' : 'file',
  }));
}
...
const DattoFileBrowser = ({ rootFolderId, siteName, onSelect, onClose }: {
  rootFolderId: string; siteName: string;
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const res = await fetch(`/api/datto?folderId=${currentFolderId}`);
  ...
  return (
    <div className="border border-indigo-200 rounded-xl overflow-hidden bg-white shadow-sm">
      ...
      <div className="bg-slate-50 border-b border-slate-100 px-4 py-2 flex items-center gap-1 flex-wrap min-h-[36px]">
        {breadcrumbs.map((crumb, i) => (
          <React.Fragment key={`${crumb.id}-${i}`}>
            ...
            <button onClick={() => setBreadcrumbs(prev => prev.slice(0, i + 1))}
              className={`text-[10px] font-black transition-colors truncate max-w-[120px] ${i === breadcrumbs.length - 1 ? 'text-indigo-700 cursor-default' : 'text-indigo-500 hover:text-indigo-700 hover:underline'}`}
              title={crumb.name}>{crumb.name}</button>
          </React.Fragment>
        ))}
      </div>
      <div className="max-h-56 overflow-y-auto">
        {loading && <div className="p-6 text-center text-[11px] font-black text-slate-400 uppercase animate-pulse">Loading…</div>}
        {!loading && apiError && (
          <div className="p-4">
            <div className="bg-rose-50 border border-rose-200 rounded-xl px-4 py-3 text-xs font-bold text-rose-700">⚠ Could not load folder: {apiError}</div>
          </div>
        )}
        {!loading && !apiError && items.length === 0 && <div className="p-6 text-center text-xs font-bold text-slate-400">This folder is empty.</div>}
        {!loading && !apiError && items.length > 0 && (
          <>
            {folders.map(item => (
              <button key={item.id} onClick={() => setBreadcrumbs(prev => [...prev, { id: item.id, name: item.name }])}
                className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-amber-50 transition-colors group border-b border-slate-50 text-left">
                <Folder size={14} className="text-amber-400 flex-shrink-0" />
                <span className="text-xs font-bold text-slate-700 group-hover:text-amber-700 flex-1 truncate">{item.name}</span>
                <ChevronRight size={12} className="text-slate-300 group-hover:text-amber-400 flex-shrink-0" />
              </button>
            ))}
            {files.map(item => (
              <button key={item.id} onClick={() => onSelect(item.name, item.id)}
                className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-indigo-50 transition-colors group border-b border-slate-50 text-left">
                <File size={14} className="text-indigo-400 flex-shrink-0" />
                <span className="text-xs font-bold text-slate-700 group-hover:text-indigo-700 flex-1 truncate">{item.name}</span>
                <span className="text-[10px] font-black text-indigo-500 bg-indigo-50 px-2 py-0.5 rounded group-hover:bg-indigo-100 flex-shrink-0">Select</span>
              </button>
            ))}
          </>
        )}
      ...
      <div className="bg-slate-50 border-t border-slate-100 px-4 py-2 flex items-center justify-between">
        <span className="text-[10px] text-slate-400">{!loading && !apiError && `${folders.length} folder${folders.length !== 1 ? 's' : ''}, ${files.length} file${files.length !== 1 ? 's' : ''}`}</span>
        <button onClick={onClose} className="text-[10px] font-black text-slate-400 hover:text-slate-600 uppercase tracking-wider">Cancel</button>
      </div>
    </div>
  );
};
...
// ─── Datto Folder Picker ──────────────────────────────────────────────────────
const DattoFolderPicker = ({ startFolderId = DATTO_ROOT_ID, startFolderName = 'Customer Documents', onSelect, onClose }: {
  ...
  return (
    <div className="border border-indigo-200 rounded-xl overflow-hidden bg-white shadow-sm">
      ...
      <div className="bg-slate-50 border-b border-slate-100 px-4 py-2 flex items-center gap-1 flex-wrap">
        {breadcrumbs.map((crumb, i) => (
          <React.Fragment key={`${crumb.id}-${i}`}>
            ...
            <button onClick={() => setBreadcrumbs(prev => prev.slice(0, i + 1))}
              className={`text-[10px] font-black truncate max-w-[120px] ${i === breadcrumbs.length - 1 ? 'text-indigo-700 cursor-default' : 'text-indigo-500 hover:text-indigo-700'}`}>
              {crumb.name}
            </button>
          </React.Fragment>
        ))}
      ...
      <div className="px-4 py-2.5 bg-indigo-50 border-b border-indigo-100">
        <button onClick={() => onSelect(current.name, current.id)}
...
const AddActionForm = ({ site, onSave, onCancel }: { site: Site; onSave: (action: Action) => void; onCancel: () => void }) => {
  ...
  const handleSave = async () => {
    ...
    const { data, error: err } = await supabase.from('actions').insert({
      site_id: site.id, title: title.trim(), description: description.trim(),
      priority, status: 'open', regulation: regulation.trim(),
      contractor: contractor.trim() || null, due_date: dueDate,
    ...
    onSave({
      id: data.id, action: data.title, description: data.description || '',
      date: data.due_date, site: site.name, who, contractor: data.contractor || '',
      ...
      regulation: data.regulation || '', notes: '', status: 'open',
    });
    setSaving(false);
  };
    ...
    <div className="bg-white rounded-2xl border border-indigo-200 shadow-lg overflow-hidden">
      ...
      <div className="p-6 space-y-5">
        ...
        <div>
          ...
          <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Additional detail..." rows={3} className={`${inputClass} resize-none`} />
        </div>
        ...
        <div>
          <label className={labelClass}>
            ...
            {site.datto_folder_id
              ? <span className="ml-2 text-indigo-400 normal-case font-bold tracking-normal">— click to browse Datto</span>
              : <span className="ml-2 text-slate-300 normal-case font-bold tracking-normal">— no Datto folder linked</span>}
          </label>
          {showFileBrowser && site.datto_folder_id ? (
            <DattoFileBrowser rootFolderId={site.datto_folder_id} siteName={site.name}
              onSelect={(name, id) => { setSourceDocName(name); setSourceDocId(id); setShowFileBrowser(false); }}
              onClose={() => setShowFileBrowser(false)} />
          ) : (
            <div onClick={() => site.datto_folder_id && setShowFileBrowser(true)}
              className={`${inputClass} flex items-center justify-between gap-2 ${site.datto_folder_id ? 'cursor-pointer hover:border-indigo-300' : 'cursor-not-allowed opacity-60'}`}>
              {sourceDocName ? (
                <>
                  <span className="flex items-center gap-2 text-indigo-700 font-bold truncate"><File size={14} className="text-indigo-400 flex-shrink-0" />{sourceDocName}</span>
                  <button onClick={e => { e.stopPropagation(); setSourceDocName(''); setSourceDocId(''); }} className="text-slate-300 hover:text-rose-400 flex-shrink-0"><X size={14} /></button>
                </>
              ) : (
                <>
                  <span className="text-slate-400">{site.datto_folder_id ? 'Click to browse documents…' : 'No Datto folder linked'}</span>
                  <FolderOpen size={16} className="text-slate-300 flex-shrink-0" />
                </>
              )}
            </div>
          )}
        ...
        <div className="flex gap-3 pt-2">
          <button onClick={handleSave} disabled={saving}
            className="flex-1 bg-indigo-600 text-white py-3 rounded-xl font-black text-sm uppercase tracking-wider hover:bg-indigo-700 disabled:opacity-50">
...
const SuperadminPanel = () => {
  ...
  const loadSites = async () => {
    const { data } = await supabase.from('sites').select('*, organisations(name)').order('name');
  ...
  const loadAssignments = async () => {
    const { data } = await supabase.from('advisor_organisations').select('*, organisations(name)').order('created_at');
  ...
  const resetOrgForm = () => { setOrgName(''); setOrgFolderId(''); setOrgFolderName(''); setShowOrgForm(false); };
  const resetSiteForm = () => { setSiteName(''); setSiteType('MANUFACTURING'); setSiteOrgId(''); setSiteFolderId(''); setSiteFolderName(''); setShowSiteForm(false); };
  const resetUserForm = () => { setUserEmail(''); setUserPassword(''); setUserRole('advisor'); setUserOrgId(''); setShowUserForm(false); };
  const resetAssignForm = () => { setAssignAdvisorId(''); setAssignOrgId(''); setShowAssignForm(false); };

  const handleCreateOrg = async () => {
    if (!orgName.trim()) { flash('Name is required', true); return; }
    const { error } = await supabase.from('organisations').insert({ name: orgName.trim(), datto_folder_id: orgFolderId || null });
    if (error) { flash(error.message, true); return; }
    flash('Organisation created!'); resetOrgForm(); loadOrgs();
  };
  ...
  const handleCreateSite = async () => {
    ...
    const { error } = await supabase.from('sites').insert({
      name: siteName.trim(), type: siteType, organisation_id: siteOrgId,
      datto_folder_id: siteFolderId || null, compliance_score: 0, trend: 0,
    });
    if (error) { flash(error.message, true); return; }
    flash('Site created!'); resetSiteForm(); loadSites();
  };
  ...
  const handleCreateUser = async () => {
    ...
    const res = await fetch('/api/admin/users', {
      ...
      body: JSON.stringify({ email: userEmail.trim(), password: userPassword, role: userRole, organisation_id: userOrgId || null }),
    });
    const data = await res.json();
    if (!res.ok) { flash(data.error, true); return; }
    flash('User created!'); resetUserForm(); loadUsers();
  };
  ...
  const handleCreateAssignment = async () => {
    ...
    if (error) { flash(error.message, true); return; }
    flash('Assignment created!'); resetAssignForm(); loadAssignments();
  };

  const handleDeleteOrg = async (id: string) => {
    if (!confirm('Delete this organisation? All its sites and actions will also be deleted.')) return;
    await supabase.from('organisations').delete().eq('id', id);
    flash('Organisation deleted'); loadOrgs(); loadSites();
  };

  const handleDeleteSite = async (id: string) => {
    if (!confirm('Delete this site? All its actions will also be deleted.')) return;
    await supabase.from('sites').delete().eq('id', id);
    flash('Site deleted'); loadSites();
  };
  ...
  const handleDeleteAssignment = async (id: string) => {
    ...
    await supabase.from('advisor_organisations').delete().eq('id', id);
    flash('Assignment removed'); loadAssignments();
  };
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            ...
            <button onClick={() => setShowOrgForm(v => !v)}
              className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-xl text-[11px] font-black uppercase tracking-wider hover:bg-indigo-700">
          ...
          {showOrgForm && (
            <div className="bg-white border border-indigo-200 rounded-2xl p-6 space-y-4">
              ...
              <div>
                ...
                <input value={orgName} onChange={e => setOrgName(e.target.value)} placeholder="e.g. Precision Engineering Ltd" className={inputClass} />
              </div>
              <div>
                <label className={labelClass}>Datto Root Folder <span className="ml-1 text-indigo-400 normal-case font-bold tracking-normal">— browse to select the client folder in Datto</span></label>
                {showOrgFolderPicker ? (
                  <DattoFolderPicker startFolderId={DATTO_ROOT_ID} startFolderName="Customer Documents"
                    onSelect={(name, id) => { setOrgFolderName(name); setOrgFolderId(id); setShowOrgFolderPicker(false); }}
                    onClose={() => setShowOrgFolderPicker(false)} />
                ) : (
                  <div onClick={() => setShowOrgFolderPicker(true)} className={`${inputClass} flex items-center justify-between gap-2 cursor-pointer hover:border-indigo-300`}>
                    {orgFolderName
                      ? <span className="flex items-center gap-2 text-indigo-700 font-bold"><Folder size={14} className="text-amber-400" />{orgFolderName} <span className="text-slate-400 font-normal text-xs">({orgFolderId})</span></span>
              ...
              <div className="flex gap-3">
                ...
                <button onClick={resetOrgForm} className="px-6 py-2.5 bg-white border border-slate-200 text-slate-500 rounded-xl text-[11px] font-black uppercase tracking-wider hover:bg-slate-50">Cancel</button>
              </div>
            </div>
          )}

          {loading ? (
            <div className="py-12 text-center text-slate-400 text-sm font-bold animate-pulse">Loading…</div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-slate-50 text-[10px] uppercase font-black text-slate-400 border-b border-slate-100">
                    <th className="px-6 py-3">Name</th>
                    <th className="px-6 py-3">Datto Folder</th>
                  ...
                  {organisations.map(org => (
                    <tr key={org.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-6 py-4 text-sm">
                        {org.datto_folder_id
                          ? <span className="flex items-center gap-1.5 text-amber-600 font-mono text-xs"><Folder size={12} /> {org.datto_folder_id}</span>
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center gap-4 flex-wrap">
            <div className="flex items-center gap-3">
              ...
              <select value={selectedOrgFilter} onChange={e => setSelectedOrgFilter(e.target.value)}
                className="px-3 py-2 border border-slate-200 rounded-xl text-xs font-bold text-slate-600 focus:outline-none bg-white">
            ...
            <div className="bg-white border border-indigo-200 rounded-2xl p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input value={siteName} onChange={e => setSiteName(e.target.value)} placeholder="e.g. Main Assembly Factory" className={inputClass} />
                </div>
                <div>
                ...
                <div className="flex gap-2 flex-wrap">
                  {SITE_TYPES.map(t => (
                    <button key={t} onClick={() => setSiteType(t)}
                      className={`px-3 py-2 rounded-xl text-[11px] font-black border transition-all ${siteType === t ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-slate-500 border-slate-200 hover:border-slate-300'}`}>
                      {t}
                    </button>
                  ))}
                </div>
              ...
              <div>
                <label className={labelClass}>
                  Datto Folder <span className="ml-1 text-slate-400 normal-case font-bold tracking-normal">(optional — if blank, uses the organisation folder)</span>
                </label>
                {showSiteFolderPicker ? (
                  <DattoFolderPicker
                    startFolderId={selectedOrgForSitePicker?.datto_folder_id || DATTO_ROOT_ID}
                    startFolderName={selectedOrgForSitePicker?.name || 'Customer Documents'}
                    onSelect={(name, id) => { setSiteFolderName(name); setSiteFolderId(id); setShowSiteFolderPicker(false); }}
                    onClose={() => setShowSiteFolderPicker(false)} />
                ) : (
                  <div onClick={() => setShowSiteFolderPicker(true)} className={`${inputClass} flex items-center justify-between gap-2 cursor-pointer hover:border-indigo-300`}>
                    {siteFolderName
                      ? <span className="flex items-center gap-2 text-indigo-700 font-bold"><Folder size={14} className="text-amber-400" />{siteFolderName}</span>
                      : <span className="text-slate-400">Click to browse (optional)…</span>}
                    <FolderOpen size={16} className="text-slate-300" />
                  </div>
                )}
              </div>
              <div className="flex gap-3">
                ...
                <button onClick={resetSiteForm} className="px-6 py-2.5 bg-white border border-slate-200 text-slate-500 rounded-xl text-[11px] font-black uppercase tracking-wider hover:bg-slate-50">Cancel</button>
              </div>
            </div>
          )}
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center">
              ...
              <p className="text-sm text-slate-400 mt-1">Add sites to your organisations above.</p>
            </div>
          ) : (
            <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
              <table className="w-full text-left">
                  ...
                  {filteredSites.map(site => (
                    <tr key={site.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-6 py-4">
                        <span className="text-[10px] font-black uppercase tracking-wider text-slate-500 bg-slate-50 border border-slate-100 px-2 py-1 rounded-lg">{site.type}</span>
                      </td>
                      <td className="px-6 py-4 text-xs font-mono">
                        {site.datto_folder_id
                          ? <span className="text-amber-600 flex items-center gap-1.5"><Folder size={12} /> {site.datto_folder_id}</span>
                          : <span className="text-slate-300">Uses org folder</span>}
                      </td>
                      <td className="px-6 py-4 text-right">
            ...
            <div className="bg-white border border-indigo-200 rounded-2xl p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="email" value={userEmail} onChange={e => setUserEmail(e.target.value)} placeholder="user@company.com" className={inputClass} />
                </div>
                <div>
                  <label className={labelClass}>Password *</label>
                  <input type="password" value={userPassword} onChange={e => setUserPassword(e.target.value)} placeholder="Min 8 characters" className={inputClass} />
                </div>
              </div>
              <div>
                <label className={labelClass}>Role *</label>
                <div className="flex gap-2">
                  {(['advisor', 'client'] as const).map(r => (
                    <button key={r} onClick={() => setUserRole(r)}
                      className={`flex-1 py-2.5 rounded-xl text-[11px] font-black border transition-all capitalize ${userRole === r ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-slate-500 border-slate-200 hover:border-slate-300'}`}>
                      {r}
                    </button>
                  ))}
                </div>
              ...
              {userRole === 'client' && (
                <div>
                  <label className={labelClass}>Organisation * <span className="ml-1 text-slate-400 normal-case font-bold tracking-normal">(required for client users)</span></label>
              ...
              <div className="flex gap-3">
                ...
                <button onClick={resetUserForm} className="px-6 py-2.5 bg-white border border-slate-200 text-slate-500 rounded-xl text-[11px] font-black uppercase tracking-wider hover:bg-slate-50">Cancel</button>
              </div>
            </div>
          )}
                    ...
                    <tr key={user.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-6 py-4">
                        <span className={`text-[10px] font-black uppercase tracking-wider px-2 py-1 rounded-lg border ${
                          user.profile?.role === 'superadmin' ? 'bg-indigo-100 text-indigo-700 border-indigo-200' :
                          user.profile?.role === 'advisor' ? 'bg-amber-100 text-amber-700 border-amber-200' :
                          'bg-emerald-100 text-emerald-700 border-emerald-200'
                        }`}>{user.profile?.role || 'unknown'}</span>
                      </td>
                      <td className="px-6 py-4 text-sm text-slate-500">
                        {user.profile?.organisation_id
                          ? organisations.find(o => o.id === user.profile.organisation_id)?.name || '—'
                          : '—'}
                      </td>
                      <td className="px-6 py-4 text-right">
                        {user.profile?.role !== 'superadmin' && (
                          <button onClick={() => handleDeleteUser(user.id)} className="text-rose-400 hover:text-rose-600 p-1.5 rounded-lg hover:bg-rose-50"><X size={14} /></button>
                        )}
                      </td>
                    </tr>
                  ))}
        ...
        <div className="space-y-4">
          ...
          {showAssignForm && (
            <div className="bg-white border border-indigo-200 rounded-2xl p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className={labelClass}>Advisor *</label>
                  <select value={assignAdvisorId} onChange={e => setAssignAdvisorId(e.target.value)} className={inputClass}>
                ...
                <div>
                  <label className={labelClass}>Organisation *</label>
                  <select value={assignOrgId} onChange={e => setAssignOrgId(e.target.value)} className={inputClass}>
                    <option value="">Select organisation…</option>
                    {organisations.map(org => <option key={org.id} value={org.id}>{org.name}</option>)}
                  </select>
                </div>
              ...
              <div className="flex gap-3">
                ...
                <button onClick={resetAssignForm} className="px-6 py-2.5 bg-white border border-slate-200 text-slate-500 rounded-xl text-[11px] font-black uppercase tracking-wider hover:bg-slate-50">Cancel</button>
              </div>
            </div>
          )}
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center">
              ...
              <p className="text-sm text-slate-400 mt-1">Assign advisors to organisations above.</p>
            </div>
          ) : (
            <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-100">
                  {assignments.map(a => (
                    <tr key={a.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-6 py-4 text-right">
                        <button onClick={() => handleDeleteAssignment(a.id)} className="text-rose-400 hover:text-rose-600 p-1.5 rounded-lg hover:bg-rose-50"><X size={14} /></button>
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <div className="bg-white rounded-3xl p-8 shadow-2xl">
          ...
          <div className="space-y-4">
            ...
            <button onClick={handleLogin} disabled={loading}
              className="w-full bg-indigo-600 text-white py-3 rounded-xl font-black text-sm uppercase tracking-wider hover:bg-indigo-700 disabled:opacity-50 mt-2">
...
export default function App() {
  ...
  useEffect(() => {
    ...
    supabase.from('profiles').select('*').eq('id', user.id).single().then(({ data }) => {
      if (data) {
        setProfile(data);
        // Superadmin defaults to admin panel
  ...
  useEffect(() => {
    ...
    // Load organisations (needed for Datto folder fallback)
  ...
  useEffect(() => {
    ...
    // Build a map of org ID → datto_folder_id for fallback
    ...
    supabase.from('sites').select('*').then(({ data }) => {
      if (data) {
        const mapped: Site[] = data.map((s: any) => ({
          ...
          lastReview: '—',
          // Use site's own folder, fall back to org folder
    ...
    supabase.from('actions').select('*').then(({ data }) => {
      if (data) {
        setAllActions(data.map((a: any) => ({
          id: a.id, action: a.title, description: a.description || '',
          date: a.due_date || '', site: sites.find(s => s.id === a.site_id)?.name || '',
          who: '', contractor: a.contractor || '',
          source: a.source_document_name || '',
          source_document_id: a.source_document_id || '',
          priority: (priorityMap[a.priority] || 'green') as Priority,
          regulation: a.regulation || '', notes: '', status: a.status as ActionStatus,
        })));
      }
    });
  }, [user, sites]);
  ...
  const handleLogout = async () => {
    ...
    setUser(null); setProfile(null); setSites([]); setOrganisations([]);
  };
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 selection:bg-indigo-100">
      ...
      <aside className="fixed left-0 top-0 h-full w-20 bg-indigo-950 flex flex-col items-center py-8 gap-10 text-indigo-300 z-20">
        ...
        <nav className="flex flex-col gap-6">
          {profile?.role === 'superadmin' && (
            <button onClick={() => setView('admin')}
              className={`p-3 rounded-xl transition-all ${view === 'admin' ? 'bg-indigo-700 text-white shadow-inner' : 'hover:text-white hover:bg-white/5'}`}
              title="Admin Panel"><Shield size={22} /></button>
          )}
          ...
          {(profile?.role === 'advisor' || profile?.role === 'client') && (
            <button onClick={() => { setView('site'); if (sites.length > 0 && !selectedSite) setSelectedSite(sites[0]); }}
          ...
          <button className="p-3 rounded-xl hover:text-white hover:bg-white/5 transition-colors" title="Settings"><Settings size={22} /></button>
        </nav>
        <div className="mt-auto flex flex-col gap-5 items-center">
          ...
          <button onClick={handleLogout} className="p-3 rounded-xl hover:text-white hover:bg-white/5 transition-colors" title="Sign out"><LogOut size={22} /></button>
        ...
        <header className="bg-white/95 backdrop-blur-sm border-b border-slate-200 px-8 py-4 flex items-center justify-between sticky top-0 z-10">
          <div className="flex items-center gap-3">
            {view === 'site' && profile?.role === 'advisor' && (
              <button onClick={() => setView('portfolio')} className="p-2 hover:bg-slate-100 rounded-lg text-slate-400"><ArrowLeft size={18} /></button>
            )}
            <div>
          ...
          <div className="flex items-center gap-5">
            ...
            {profile?.role === 'advisor' && (
              <div className="hidden lg:flex bg-slate-100 p-1 rounded-xl">
                <button onClick={() => { setView('portfolio'); setSelectedSite(null); }}
                  className={`px-4 py-1.5 text-[10px] font-black uppercase rounded-lg transition-all ${view === 'portfolio' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-400 hover:text-slate-600'}`}>Dashboard</button>
                <button onClick={() => { setView('site'); if (sites.length > 0 && !selectedSite) setSelectedSite(sites[0]); }}
                  className={`px-4 py-1.5 text-[10px] font-black uppercase rounded-lg transition-all ${view === 'site' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-400 hover:text-slate-600'}`}>Action Plan</button>
              </div>
            )}
          </div>
        </header>
            ...
            <div className="space-y-8 animate-in fade-in duration-500">
              ...
              <div className="flex border-b border-slate-200 gap-6">
                {[{ key: 'analytics', label: 'Visual Analytics', icon: <BarChart3 size={14} /> }, { key: 'data', label: 'Division Registry', icon: <Building2 size={14} /> }].map(tab => (
                  <button key={tab.key} onClick={() => setDashboardTab(tab.key as 'analytics' | 'data')}
                    className={`pb-4 px-1 text-[11px] font-black uppercase tracking-widest flex items-center gap-2 border-b-2 transition-all ${dashboardTab === tab.key ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-400 hover:text-slate-600'}`}>
                    {tab.icon} {tab.label}
                  </button>
                ))}
              </div>
                      ...
                      {sites.map(site => (
                        <div key={site.id} className="group cursor-pointer" onClick={() => handleSiteClick(site)}>
                          <div className="flex justify-between items-center mb-2">
                            <div className="flex items-center gap-3">
                              <div className="w-7 h-7 bg-slate-50 rounded-lg flex items-center justify-center text-slate-400 group-hover:bg-indigo-600 group-hover:text-white transition-all text-xs">{getSiteIcon(site.type, 14)}</div>
                              <span className="text-sm font-bold text-slate-700 group-hover:text-indigo-700">{site.name}</span>
                            </div>
                            <div className="flex items-center gap-3">
                          ...
                          <div className="w-full bg-slate-100 h-3 rounded-full overflow-hidden shadow-inner">
                            <div className={`h-full rounded-full transition-all duration-1000 ${site.compliance >= 90 ? 'bg-emerald-500' : site.compliance >= 75 ? 'bg-indigo-500' : 'bg-rose-500'}`} style={{ width: `${site.compliance}%` }} />
                          </div>
                        </div>
                      ))}
                    ...
                    <div className="flex-1 flex flex-col justify-center items-center">
                      <div className="relative w-36 h-36 flex items-center justify-center mb-6">
                        <svg className="w-full h-full -rotate-90" viewBox="0 0 160 160">
                          ...
                          <circle cx="80" cy="80" r="70" stroke="#f43f5e" strokeWidth="16" fill="none" strokeDasharray="440" strokeDashoffset="418" strokeLinecap="round" />
                        </svg>
                      ...
                      <div className="w-full space-y-2.5">
                        {[
                          ...
                          <div key={item.label} className={`flex items-center justify-between text-xs font-black px-4 py-2.5 rounded-xl border ${item.color}`}>
                            <span>{item.label}</span><span className="text-base font-black">{item.count}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    {sites.map(site => (
                      <div key={site.id} className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm cursor-pointer hover:border-indigo-300 hover:shadow-md transition-all group" onClick={() => handleSiteClick(site)}>
                        <div className="flex items-start justify-between mb-4">
                          <div className="w-10 h-10 bg-slate-50 rounded-xl flex items-center justify-center text-slate-400 group-hover:bg-indigo-600 group-hover:text-white transition-all">{getSiteIcon(site.type)}</div>
                      ...
                      {sites.map(site => (
                        <tr key={site.id} className="hover:bg-indigo-50/30 cursor-pointer group" onClick={() => handleSiteClick(site)}>
                          <td className="px-8 py-5">
                            <div className="flex items-center gap-3">
                              <div className="w-10 h-10 bg-slate-50 rounded-xl flex items-center justify-center text-slate-400 group-hover:bg-indigo-600 group-hover:text-white transition-all shadow-sm">{getSiteIcon(site.type)}</div>
          ...
          {view === 'site' && selectedSite && (
            <div className="space-y-6 animate-in slide-in-from-right-8 duration-400">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {profile?.role === 'advisor' && (
                  <button onClick={() => setShowAddAction(true)}
                    className="ml-2 flex items-center gap-2 px-5 py-2 bg-indigo-600 text-white rounded-xl text-[11px] font-black uppercase tracking-wider hover:bg-indigo-700 shadow-sm">
              ...
              {showAddAction && selectedSite && (
                <AddActionForm site={selectedSite} onSave={handleActionSaved} onCancel={() => setShowAddAction(false)} />
              )}
                  ...
                  <div className="flex gap-3 flex-wrap">
                    {sites.map(site => (
                      <button key={site.id} onClick={() => setSelectedSite(site)}
                        className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-black border transition-all ${selectedSite.id === site.id ? 'bg-indigo-600 text-white border-indigo-600 shadow-md' : 'bg-white text-slate-600 border-slate-200 hover:border-indigo-300 hover:text-indigo-600'}`}>
                        {getSiteIcon(site.type, 14)} {site.name}
                      </button>
                    ))}
                  </div>
                </div>
